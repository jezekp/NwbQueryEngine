...
public class QueryParser implements Parser {
    ...
    private Expression parseInternal(Expression e, String operator) {
        ...
        while (brackets.find()) {
            ...
            node.setRightSide(parseInternal(new Expression(value, node), AND_OR));
        }
        ...
        if (!operator.equals(ASSIGN)) {
            ...
            parseSubString(input, node, "((?<=" + operator + ")|(?=" + operator + "))");
        }
        return node;
    }
    ...
    private void parseSubString(String input, Expression node, String delimiter) {
        ...
        if (st.length > 1) {
            Expression left = new Expression(st[0], st[1], node);
            Expression right = new Expression(st[2], st[1], node);
            if (isOthers) {
                node.setLeftSide(left);
                node.setRightSide(right);
            } else {
                node.setLeftSide(parseInternal(left, OTHERS));
                node.setRightSide(parseInternal(right, AND_OR));
            }
            ...
            Expression e = new Expression(st[0], null, node);
            if (!isOthers) {
                node.setLeftSide(parseInternal(e, OTHERS));
            } else {
                node.setLeftSide(e);
            }
        }
